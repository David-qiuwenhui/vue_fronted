...
<script>
import { mapGetters, mapState } from "_vuex@3.6.2@vuex";
...
export default {
    ...
    methods: {
        ...
        // 移除自定义搜索keyword
        removeKeyword() {
            // 清空keyword相关属性 设置undefined能够提高网络性能
            this.searchParams.keyword = undefined;
            // 再次发送请求
            this.getData();
            // 利用全局事件总线清除兄弟组件搜索框keyword内容
            this.$bus.$emit("clear");
            // 更新路由跳转
            if (this.$route.query) {
        ...
        getAttrs(attrs, attrValue) {
            let attrParams = `${attrs.attrId}:${attrValue}:${attrs.attrName}`;
    ...
    // 数据监听
    watch: {
        // 监听组件实例身上属性的值是否发生变化 如果发生变化 再次发起请求
        $route(newValue, oldValue) {
            ...
            // 再次发起ajax请求
            this.getData();
            // 请求完毕之后 清空三级分类的id 方便下次属性值变化时正确接收和发送
            // 分类名字和关键字不用清空 每次路由发生变化时 都会赋予新的数据
            this.searchParams.category1Id = undefined;
            this.searchParams.category2Id = undefined;
            this.searchParams.category3Id = undefined;
        },
